...
if [[ -z "${BASH_VERSINFO:-}" || "${BASH_VERSINFO[0]}" -lt 4 ]]; then
  ...
  echo "Error: recommend_skills.sh requires bash 4+." >&2
  exit 1
fi
set -euo pipefail
...
# If you clone the claude-skills-library repository locally you can also run the script directly.

REPO_SLUG="daryllundy/claude-skills-library"
DEFAULT_BRANCH="main"
CACHE_DIR_DEPRECATION_NOTIFIED=false
...
resolve_env_value() {
  ...
  if [[ -n "$old_value" ]]; then
    warn_deprecated "$old_name is deprecated; use $new_name instead."
    printf '%s\n' "$old_value"
    return
  fi
...
CLAUDE_SKILLS_BRANCH="$(resolve_env_value CLAUDE_SKILLS_BRANCH CLAUDE_AGENTS_BRANCH "$DEFAULT_BRANCH")"
CLAUDE_SKILLS_REPO="$(resolve_env_value CLAUDE_SKILLS_REPO CLAUDE_AGENTS_REPO "https://raw.githubusercontent.com/${REPO_SLUG}")"
BASE_URL="${CLAUDE_SKILLS_REPO}/${CLAUDE_SKILLS_BRANCH}/.claude/skills"
...
Options:
  ...
  --branch NAME          Override the claude-skills-library branch to download from.
  --repo URL             Override the base raw URL for the claude-skills-library repository.
  --patterns-dir DIR     Specify custom pattern directory location.
  -h, --help             Show this help message.

Environment variables:
  CLAUDE_SKILLS_BRANCH    Override the branch (default: main).
  CLAUDE_SKILLS_REPO      Override the raw content base URL.
  SKILL_PATTERNS_DIR      Override the pattern directory location.
  CLAUDE_SKILLS_CACHE     Override the cache directory location.

Legacy compatibility:
  CLAUDE_AGENTS_BRANCH, CLAUDE_AGENTS_REPO, AGENT_PATTERNS_DIR, and
  CLAUDE_AGENTS_CACHE are still honored temporarily with deprecation warnings.
...
log() {
  printf '[skill-setup] %s\n' "$*"
}

resolve_cache_dir() {
  local explicit_cache="${CACHE_DIR:-}"
  local new_cache_env="${CLAUDE_SKILLS_CACHE:-}"
  local old_cache_env="${CLAUDE_AGENTS_CACHE:-}"
  local default_cache="${XDG_CACHE_HOME:-$HOME/.cache}/claude-skills-library"
  local legacy_cache="${XDG_CACHE_HOME:-$HOME/.cache}/claude-agents"
  ...
  if [[ -n "$new_cache_env" ]]; then
    printf '%s\n' "$new_cache_env"
    return
  fi

  if [[ -n "$old_cache_env" ]]; then
    warn_deprecated "CLAUDE_AGENTS_CACHE is deprecated; use CLAUDE_SKILLS_CACHE instead."
    printf '%s\n' "$old_cache_env"
    return
  fi

  if [[ -d "$legacy_cache" && ! -d "$default_cache" && "$CACHE_DIR_DEPRECATION_NOTIFIED" == false ]]; then
    CACHE_DIR_DEPRECATION_NOTIFIED=true
    warn_deprecated "Using legacy cache directory $legacy_cache. The new default is $default_cache."
  fi
...
get_legacy_cache_path() {
  ...
  echo "${legacy_cache}/$(basename "$cache_file")"
}
...
fetch_with_cache() {
  ...
  if [[ $force_refresh == false ]] && [[ "$cache_file" != "$legacy_cache_file" ]] && is_cache_fresh "$legacy_cache_file" "$expiry"; then
    if [[ ${VERBOSE:-false} == true ]]; then
      log "Using legacy cached version of $(basename "$url") from $(dirname "$legacy_cache_file")"
    fi
    cp "$legacy_cache_file" "$output"
    cp "$legacy_cache_file" "$cache_file" 2>/dev/null || true
    return 0
  fi
    ...
    if [[ $attempt -lt $max_attempts ]]; then
      ...
      case "$http_code" in
        000)
          ...
          ;;
        404)
          echo "Troubleshooting: The skill file was not found. It may have been removed or renamed" >&2
          ;;
        403|401)
...
display_categorized_results() {
  ...
  echo ""
  echo "═══════════════════════════════════════════════════════════════════════"
  echo "                     Skill Recommendation Results"
  echo "═══════════════════════════════════════════════════════════════════════"
  echo ""
  echo "Found ${#recommended_agents[@]} recommended skills"
  echo ""
...
render_agent_list() {
  ...
  echo "═══════════════════════════════════════════════════════════════════════"
  echo "            Skill Recommendation - Interactive Mode"
  echo "═══════════════════════════════════════════════════════════════════════"
  echo ""
...
interactive_selection() {
  ...
  log "Selected ${#recommended_agents[@]} skills for installation"
}
...
setup_patterns_directory() {
  ...
  # Priority 2: Environment variable
  elif [[ -n "${SKILL_PATTERNS_DIR:-}" ]]; then
    patterns_dir="$SKILL_PATTERNS_DIR"
    if [[ ! -d "$patterns_dir" ]]; then
      echo "Error: Pattern directory specified via SKILL_PATTERNS_DIR does not exist: $patterns_dir" >&2
    ...
    warn_deprecated "AGENT_PATTERNS_DIR is deprecated; use SKILL_PATTERNS_DIR instead."
...
load_pattern_files() {
  ...
  if [[ ${VERBOSE:-false} == true ]]; then
    ...
    echo "✓ Loaded $total_agents skill detection patterns from $loaded_files file(s)" >&2
  fi
...
load_detection_patterns() {
  ...
  if [[ -n "$yaml_file" ]]; then
    if parse_yaml_patterns "$yaml_file"; then
      ...
      if [[ "$count" -gt 0 ]]; then
        echo "✓ Loaded $count skill detection patterns from ${yaml_file##*/}" >&2
        return 0
      fi
    fi
  fi
